...
function mergeObjects(objects) {
  ...
  objects.forEach((targetObj) => {
    Object.entries(targetObj).forEach(([key, value]) => {
      const keyExists = Object.prototype.hasOwnProperty.call(merge, key);
      let sum = value;
...
function compareObjects(obj1, obj2) {
  ...
  return Object.entries(obj1).every(([firstObjKey, firstObjValue]) => {
    const hasSameKey = Object.prototype.hasOwnProperty.call(obj2, firstObjKey);

    if (!hasSameKey) return false;

    if (firstObjValue !== obj2[firstObjKey]) return false;
...
function sellTickets(queue) {
  ...
  while (queue.length > 0) {
    const moneyFromCustomer = queue.shift();

    if (moneyFromCustomer === TICKET_PRICE) {
      totalMoney += moneyFromCustomer;
    } else {
      const change = moneyFromCustomer - TICKET_PRICE;
...
function group(array, keySelector, valueSelector) {
  ...
  array.forEach((item) => {
    ...
    const groupedArray = result.get(key) ?? [];

    groupedArray.push(value);

    result.set(key, groupedArray);
  });
